...
class UserController extends Controller
{
    ...
    {
        $users = User::with(['warehouse', 'warehouse.primaryCash', 'contact'])
            ->when($request->search, function ($query, $search) {
                // Dibungkus closure agar query GROUPING-nya aman
    ...
    {
        $user = User::with(['contact', 'warehouse'])->find($id);
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            // 1. Simpan ID contact lama sebelum di-update
            $oldContactId = $user->contact_id;
            $newContactId = $request->contact;
            ...
            if ($oldContactId && $oldContactId != $newContactId) {
                // Cari contact lama langsung via Model Contact agar aman
                \App\Models\Contact::where('id', $oldContactId)->update([
                    'email' => null,
                    'user_id' => null // Opsional: lepas juga hubungannya jika diperlukan
                ]);
            }

            // 4. Update email dan user_id di contact yang BARU
            if ($newContactId) {
                \App\Models\Contact::where('id', $newContactId)->update([
                    'email' => $request->email,
                    'user_id' => $user->id
                ]);
            }
    ...
    {
        $users = User::with(['contact', 'warehouse'])->orderBy('name', 'asc')->get();
